    ...
    <form action="/obat/{{ $obat->id }}" method="POST">
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div class="mb-4">
                ...
                <select name="relasional_obat_id" required 
                    ...
                    <option value="{{ $kategori->id }}" {{ old('relasional_obat_id', $obat->relasional_obat_id) == $kategori->id ? 'selected' : '' }}>
        ...
        <div class="mb-4">
            ...
            <textarea name="deskripsi" rows="2" 
                class="w-full px-4 py-2 border border-slate-300 rounded focus:outline-none focus:border-blue-500">{{ old('deskripsi', $obat->deskripsi) }}</textarea>
        </div>

        <div class="grid grid-cols-3 gap-4">
            <div class="mb-4">
                <label class="block text-slate-700 font-semibold mb-2">Stok</label>
                <input type="number" name="stok" value="{{ old('stok', $obat->stok) }}" required min="0" 
                    class="w-full px-4 py-2 border border-slate-300 rounded focus:outline-none focus:border-blue-500">
                @error('stok')
                <span class="text-red-600 text-sm">{{ $message }}</span>
                @enderror
            ...
            <div class="mb-4">
                ...
                <input type="number" name="harga_beli" value="{{ old('harga_beli', $obat->harga_beli) }}" required min="0" step="0.01" 
                    class="w-full px-4 py-2 border border-slate-300 rounded focus:outline-none focus:border-blue-500">
                @error('harga_beli')
                <span class="text-red-600 text-sm">{{ $message }}</span>
                @enderror
            ...
            <div class="mb-4">
                ...
                <input type="number" name="harga_jual" value="{{ old('harga_jual', $obat->harga_jual) }}" required min="0" step="0.01" 
                    class="w-full px-4 py-2 border border-slate-300 rounded focus:outline-none focus:border-blue-500">
                @error('harga_jual')
                <span class="text-red-600 text-sm">{{ $message }}</span>
                @enderror
        ...
        <div class="mb-4">
            ...
            <input type="date" name="tanggal_kadaluarsa" value="{{ old('tanggal_kadaluarsa', $obat->tanggal_kadaluarsa?->format('Y-m-d')) }}" 
                class="w-full px-4 py-2 border border-slate-300 rounded focus:outline-none focus:border-blue-500">
        </div>
